...
<!--
	...
	Conversión de Common Basic Components
-->
<!--
...
<xsl:stylesheet version="1.0" xmlns:ubl="urn:oasis:names:specification:ubl:schema:xsd:Invoice-2" 
	...
	<xsl:template match="cac:Country/cbc:IdentificationCode">
		...
		<xsl:element name="CountryCode">
			<xsl:value-of select="document('CountryIdentificationCode-2.0_facturae30.gc')/gc:CodeList/SimpleCodeList/Row[Value[@ColumnRef='code']/SimpleValue=$countryCode]/Value[@ColumnRef='code3']/SimpleValue"/>
		</xsl:element>
	</xsl:template>
	...
	<xsl:template mode="legal" match="cbc:Note">
		<xsl:element name="LegalReference">
			<xsl:value-of select="."/>
		</xsl:element>
	</xsl:template>
	<xsl:template mode="additional" match="cbc:Note">
	...
	<xsl:template match="cbc:InvoicedQuantity/@unitCode">
		<!-- TODO: Map UnitOfMeasures
		...
		<xsl:element name="PaymentMeans">
			...
			<xsl:variable name="paymentCode"><xsl:value-of select="."/></xsl:variable>
				<xsl:choose>
					...
					<xsl:when test='$paymentCode="30"'>04</xsl:when>
